import sensor, image, time, lcd, micropython, gc, os
from fpioa_manager import fm
from maix import GPIO, KPU, utils
gc.collect();
...
try:
    while 1:
        ...
        for lp in lps:
            ...
            del lp_img
            del resize_img
        img.draw_string(10, 10, "K1/BOOT: Snap Plate", color=(0, 255, 255), scale=1)
        # 检测快门按键 (支持 K1 与 BOOT)
        if detector_key.check_pressed():
            save_path = "%s/car_%03d.jpg" % (save_dir, snap_id)
            img.save(save_path)
            print("[实时快门] 按键触发！已抓拍保存车牌图: %s" % save_path)
