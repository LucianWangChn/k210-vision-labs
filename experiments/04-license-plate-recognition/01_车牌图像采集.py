...
# 注册双按键快门 (同时支持 K1 和 BOOT 键，按任意一个均可触发快门)
...
try:
    ...
    for f in files:
        if f.startswith("car_") and f.endswith(".jpg"):
            num_str = f.replace("car_", "").replace(".jpg", "")
            if num_str.isdigit():
...
print("==================================================")
print("===      车牌数据采集程序 (防开机误拍版)       ===")
print("  * 专用快门按键: K1 (IO16)")
print("  * 保存路径: %s" % save_dir)
print("  * 下张编号: car_%03d.jpg" % photo_id)
print("==================================================")
...
while True:
    ...
    if detector_key.check_pressed():
        file_path = "%s/car_%03d.jpg" % (save_dir, photo_id)
        img.save(file_path)
        print("[快门] 触发按键 | 已成功保存车牌图: %s" % file_path)
        
        img.draw_rectangle(0, 0, 320, 240, color=(0, 255, 0), thickness=6)
        img.draw_string(15, 15, "SAVED: car_%03d.jpg" % photo_id, color=(0, 255, 0), scale=2)
        lcd.display(img)
        
        photo_id += 1
        time.sleep_ms(400)
    else:
        img.draw_string(10, 10, "Press K1 / BOOT to Snap", color=(255, 255, 0), scale=1)
        img.draw_string(10, 26, "Next: car_%03d.jpg" % photo_id, color=(0, 255, 255), scale=1)
        lcd.display(img)
